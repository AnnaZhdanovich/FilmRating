...
/**
 * Class {@code AddFilmCommand} performs the request (command) to the changing
 * role of the user.
 * 
...
public class ChangeUserRoleCommand implements ICommand {
	...
	public void execute(HttpServletRequest request, Carrier carrier) throws CommandException {
		...
		try {
			...
			if (Validator.check(idUser) && Validator.check(role)) {
				...
				result = admin.changeRole(Integer.parseInt(idUser), role);
