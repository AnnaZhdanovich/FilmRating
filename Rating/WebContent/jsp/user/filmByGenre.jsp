			...
			<c:forEach var="film" items="${requestScope.films}">
				<div class="wrap_film">
					...
					<div class="wrap_film_data">
						...
						<div class="fm_rating">&#9734; ${film.rating}</div>
			...
			<div class="delimetr">
				<c:if test="${not empty requestScope.films }">
					<c:if test="${sessionScope.currentPage != 1}">
						<td><a class="link"
							href="/Rating/MainController?command=find_films_by_genre&genre=${sessionScope.genre}&page=${sessionScope.currentPage-1}">${previous}</a></td>
					</c:if>
						...
						<tr>
							<c:forEach begin="1" end="${sessionScope.noOfPages}" var="i">
								<c:choose>
									...
									<c:otherwise>
										<td><a class="link"
											href="/Rating/MainController?command=find_films_by_genre&genre=${sessionScope.genre}&page=${i}">${i}</a></td>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</tr>
					...
					<c:if test="${sessionScope.currentPage lt sessionScope.noOfPages}">
						<td><a class="link"
							href="/Rating/MainController?command=find_films_by_genre&genre=${sessionScope.genre}&page=${sessionScope.currentPage + 1}">${next}</a></td>
					</c:if>
				</c:if>
			</div>
		</div>
