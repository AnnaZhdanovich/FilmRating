		...
		<div class="main">
			...
			<c:if test="${not empty requestScope.films }">
				<c:if test="${sessionScope.currentPage != 1}">
					<td><a class="link"
						href="/Rating/MainController?command=find_list_of_films&title=${sessionScope.title}&country=${sessionScope.country}&year=${sessionScope.year }&actor=${sessionScope.actor}&producer=${sessionScope.producer }&genre=${sessionScope.genre }&goal=${sessionScope.goal}&page=${sessionScope.currentPage-1}">${previous}</a></td>
				</c:if>
					...
					<tr>
						<c:forEach begin="1" end="${sessionScope.noOfPages}" var="i">
							<c:choose>
								...
								<c:otherwise>
									<td><a class="link"
										href="/Rating/MainController?command=find_list_of_films&title=${sessionScope.title}&country=${sessionScope.country}&year=${sessionScope.year }&actor=${sessionScope.actor}&producer=${sessionScope.producer }&genre=${sessionScope.genre }&goal=${sessionScope.goal}&page=${i}">${i}</a></td>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</tr>
				...
				<c:if test="${sessionScope.currentPage lt sessionScope.noOfPages}">
					<td><a class="link"
						href="/Rating/MainController?command=find_list_of_films&title=${sessionScope.title}&country=${sessionScope.country}&year=${sessionScope.year }&actor=${sessionScope.actor}&producer=${sessionScope.producer }&genre=${sessionScope.genre }&goal=${sessionScope.goal}&page=${sessionScope.currentPage + 1}">${next}</a></td>
				</c:if>
			</c:if>
		</div>
	</div>
